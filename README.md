...
```
It means that the appearances score of each letter is calculated within the shortlist rather than only initially with the whole dictionary. However this has been turned off because appeared to have poorer performances in time and guesses.
#### shortlist.py
...
It is true that some information is redundant in this code as testifies the long list of used structures. However the assignements to new variables have been kept minimal in the code and the overall used memory (<500MB) is perfectly manageable by a personal laptop, even in this _auto_runs.py_ case, and does not properly threatens the speed of the code.
